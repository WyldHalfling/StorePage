...
class ValidateRequest {
     ...
     * @param $policies, the rules that validation will satisify
     */
    public function abide(array $dataAndValues, array $policies) {
        foreach ($dataAndValues as $column => $value) {
            if (in_array($column, array_keys($policies))) {
                // do validation
                self::doValidation(['column' => $column, 'value' => $value, 'policies' => $policies[$column]]);
            }
        }
    }
    ...
    /**
     * Applies to ever rule method
     * 
     * @param $column, field name or column
     * @param $value, value passed into the form
     * @param $policy, the rule that we set e.g. minLen = 5
